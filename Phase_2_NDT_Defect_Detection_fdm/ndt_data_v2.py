...
def k_distribution_ground_truth(X, Y):
    ...
    # rect
    mask_rect = ((X > 0.64) & (X < 0.76) & (Y > 0.56) & (Y < 0.74))
    K_val[mask_rect] = 0.2
    return K_val
...
def visualize_data(x, y, t, U, K, K_raw, save_dir):
    ...
    rect = plt.Rectangle((0.64, 0.56), 0.12, 0.18, color='red', fill=False, linewidth=2)
    ...
    for idx, t_idx in enumerate(time_indices):
        ...
        rect = plt.Rectangle((0.64, 0.56), 0.12, 0.18, color='cyan', fill=False, linewidth=2, linestyle='--')
    ...
    rect = plt.Rectangle((0.64, 0.56), 0.12, 0.18, color='red', fill=False, linewidth=2, label='Defect')
    ax.add_patch(circle)
    ax.add_patch(rect)
